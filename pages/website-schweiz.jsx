...
const reviews = [
  ...
  const [blueFormData, setBlueFormData] = useState({
  name: "",
  email: "",
  subject: "",
  message: "",
});
const [blueFormErrors, setBlueFormErrors] = useState({});
const [isBlueFormSubmitted, setIsBlueFormSubmitted] = useState(false);

const handleBlueFormChange = (e) => {
  setBlueFormData({ ...blueFormData, [e.target.name]: e.target.value });
};

const handleBlueFormSubmit = async (e) => {
  e.preventDefault();

  const newErrors = {};
  if (!blueFormData.name) newErrors.name = "Bitte Ausfüllen";
  if (!blueFormData.email) newErrors.email = "Bitte Ausfüllen";
  if (!blueFormData.subject) newErrors.subject = "Bitte Ausfüllen";
  if (!blueFormData.message) newErrors.message = "Bitte Ausfüllen";

  if (Object.keys(newErrors).length > 0) {
    setBlueFormErrors(newErrors);
    return;
  }

  try {
    const response = await fetch("/api/sendBlueFormMail", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(blueFormData),
    });
  ...
  return (
    <div className="px-4">
      ...
      <p className="text-[#BFBFBF] font-matt text-[12px] lg:text-[16px] font-normal lg:leading-[28px] ml-1">
        Bereits von über 144 Startups & KMU's geschätzt
      </p>
    </div>
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="w-full">
            ...
            />
            {blueFormErrors.name && (
              <span className="text-red-400 text-sm">{blueFormErrors.name}</span>
            )}
          </div>
          ...
          <div className="w-full">
            <input
              ...
              name="email"
              placeholder="Email"
              value={blueFormData.email}
              onChange={handleBlueFormChange}
              className="w-full p-3 rounded border border-white bg-transparent text-white placeholder-white"
            />
            {blueFormErrors.email && (
              <span className="text-red-400 text-sm">{blueFormErrors.email}</span>
            )}
          </div>
        </div>
<div>
    ...
    <span className="text-red-400 text-sm">{blueFormErrors.subject}</span>
        ...
        <div>
          <textarea
            ...
            rows="4"
            value={blueFormData.message}
            onChange={handleBlueFormChange}
            className="w-full p-3 rounded border border-white bg-transparent text-white placeholder-white"
          />
          {blueFormErrors.message && (
            <span className="text-red-400 text-sm">{blueFormErrors.message}</span>
          )}
        </div>
...
<section className="mt-[20px] mb-[160px] lg:mb-[300px]">
  <h2 className="text-center text-[50px] lg:text-[64px] lg:leading-[72px] font-[300] font-matt text-black mb-16">
...
überprüfen Sie Ihren Posteingang.
  ...
  <div>
    ...
    <button onClick={scrollToContactForm} className="btnfix lg:ml-[270px] lg:mt-[240px] custom-1060 group relative font-matt flex items-center justify-center px-1 py-1 bg-transparent border-2 border-[#0009FF] text-[#0009FF] rounded-[10px] font-medium text-[20px] transition duration-100 ease-in-out overflow-hidden">
            {/* SVG Background */}
