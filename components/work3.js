...
const ServicesSection3 = () => {
  ...
  const swiperSectionRef = useRef(null); // Reference to the swiper section
  const [scrolling, setScrolling] = useState(false); // Prevent multiple scroll triggers
  const [isLastSlide, setIsLastSlide] = useState(false); // To detect when it's the last slide
  ...
  const handleScroll = (e) => {
    ...
    if (swiperRef.current) {
      if (e.deltaY > 0) {
        ...
        if (isLastSlide) {
          // If it's the last slide, allow the page to scroll down to the next section
  ...
  }, [scrolling, isLastSlide]);

  useEffect(() => {
    // Update `isLastSlide` state when Swiper reaches the last slide
    if (swiperRef.current) {
    ...
    <>
      <div
        ...
        style={{
          position: "sticky",
          top: "0",
          height: "100vh", // Full viewport height for the swiper section
      ...
      >
        <Swiper
          ...
          mousewheel={{ forceToAxis: true }} // Mouse wheel controlled
          className="swiper-cards"
          style={{
            width: "100%", // Full width of the viewport
            height: "100%", // Full height for visibility
          }}
        >
         <SwiperSlide>
          <motion.section
    ...
    >
      ...
      >
        <div className="transition-transform duration-500 ease-out transform group-hover:scale-100">
          ...
          {/* Additional slides can be added */}
        </Swiper>
      </div>
...
export { HeaderSection3, ServicesSection3};
